...
def RandomMaskRect(h, w, hole_range=[0,1]):
    coef = min(hole_range[0] + hole_range[1], 1.0)
    H, W = h, w
    while True:
        mask = np.ones((h, w), np.uint8)
        def Fill(max_hsize, max_wsize):
            w, h = np.random.randint(max_wsize), np.random.randint(max_hsize)
            ww, hh = w // 2, h // 2
            x, y = np.random.randint(-ww, W - w + ww), np.random.randint(-hh, H - h + hh)
            mask[max(y, 0): min(y + h, H), max(x, 0): min(x + w, W)] = 0
        def MultiFill(max_tries, max_hsize, max_wsize):
            for _ in range(np.random.randint(max_tries)):
                Fill(max_hsize, max_wsize)
        MultiFill(int(10 * coef), h // 2, w // 2)
        MultiFill(int(5 * coef), h, w)
        mask = np.logical_and(mask, 1 - RandomBrushRect(int(20 * coef), h, w, min_width = 10, max_width = 40))
        hole_ratio = 1 - np.mean(mask)
        if hole_range is not None and (hole_ratio <= hole_range[0] or hole_ratio >= hole_range[1]):
            continue
        # return mask[np.newaxis, ...].astype(np.float32)

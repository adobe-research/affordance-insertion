    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        if isinstance(cond, dict):
            if self.model.conditioning_key == 'hybrid':
                # concat_cond = torch.cat([cond['concat'][key] for key in sorted(cond['concat'])], axis=1)
                # crossattn_cond = torch.cat([cond['crossattn'][key] for key in sorted(cond['crossattn'])], axis=1)

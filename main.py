...
class DataModuleFromConfig(pl.LightningDataModule):
    ...
    def _fid_cond_dataloader(self, shuffle=False):
        if isinstance(self.datasets['fid_cond'], Txt2ImgIterableBaseDataset) or self.use_worker_init_fn:
            init_fn = worker_init_fn
        else:
            init_fn = None
        return DataLoader(self.datasets["fid_cond"],
                          batch_size=self.batch_size,
                          num_workers=self.num_workers,
                          worker_init_fn=init_fn,
                          shuffle=shuffle)
    
    def _fid_hall_dataloader(self, shuffle=False):
        if isinstance(self.datasets['fid_hall'], Txt2ImgIterableBaseDataset) or self.use_worker_init_fn:
            init_fn = worker_init_fn
        else:
            init_fn = None
        return DataLoader(self.datasets["fid_hall"],
                          batch_size=self.batch_size,
                          num_workers=self.num_workers,
                          worker_init_fn=init_fn,
                          shuffle=shuffle)
...
if __name__ == "__main__":
    ...
    if opt.distributed:
        ...
        opt.nodes_info = json.loads(opt.nodes_info)
        print("PARSED SUCCESSFULLY")
        os.environ["MASTER_ADDR"] = opt.nodes_info["ip_list"][0]
    ...
    try:
        ...
        # data.prepare_data() # slightly slow, no mem overhead
        # data.setup() # slightly but little bit more slow, low mem overhead
        # print("#### Data #####")
